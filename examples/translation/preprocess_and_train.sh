...
where:\n
    ...
    -j  set the number decrement lines per experiment (if not set only one training corpus will be created)
EndOfMessage
}
...
if isarray "language"; then
  for l in "${language[@]}"; do
      ...
      for ((i = max_lines ; i > decrement ; i=i-decrement)); do
        python3 "training_model.py" "-l $l" "-p" "-cs $max_lines" "-mt "baseline""
      done
  done
else
    echo "Starting to train models for the $language language." 
    for ((i = max_lines ; i > decrement ; i=i-decrement)); do
        python3 "training_model.py" "-l" "$tgt" "-p" "-cs" "$max_lines" "-mt" ""baseline""
    done
fi