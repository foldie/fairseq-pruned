...
class FairseqTask(object):
    ...
    def train_step(
        ...
        optimizer.counter += 1
              #transformer = model
              #y = transformer(Variable(sample['net_input']['src_tokens']), sample['net_input']['src_lengths'],
              #                  sample['net_input']['prev_output_tokens'])
              #g = mmake_dot(y[0], dict(list(model.named_parameters())))
              #g.render("./pdfs/blu.gv.pdf", view = False)
              #k = make_dot(transformer(sample['net_input']['src_tokens'], sample['net_input']['src_lengths'],sample['net_input']['prev_output_tokens'] )[0], params=dict(transformer.named_parameters()))
              #optimizer.plot_grad_flow(model)
              #k.render("./pdfs/new.gv.pdf", view=False)
              #optimizer.plot_loss()
